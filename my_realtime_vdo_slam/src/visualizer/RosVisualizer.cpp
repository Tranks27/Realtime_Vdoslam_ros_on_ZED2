...
namespace VDO_SLAM {
    ...
    RosVisualizer::RosVisualizer(VisualizerParamsPtr& params_)
        :   Visualizer2D(params_),
            ...
            ros::SubscribeOptions vdo_slam_subscriber_options =
                ros::SubscribeOptions::create<my_realtime_vdo_slam::VdoSlamScene>(
            ...
            ros::SubscribeOptions vdo_slam_map_subscriber_options =
                ros::SubscribeOptions::create<my_realtime_vdo_slam::VdoSlamMap>(
                    "/vdoslam/output/map",
                    kMaxSceneQueueSize,
    ...
    void RosVisualizer::slam_scene_callback(const my_realtime_vdo_slam::VdoSlamSceneConstPtr& slam_scene) {
        my_realtime_vdo_slam::VdoSlamSceneConstPtr slam_scene_ptr = slam_scene;
        SlamScenePtr scene = Scene::create<my_realtime_vdo_slam::VdoSlamSceneConstPtr>(slam_scene_ptr);
    ...
    void RosVisualizer::reconstruct_scenes_callback(const my_realtime_vdo_slam::VdoSlamMapConstPtr& map) {
        // ROS_INFO_STREAM("Recieved slam map updated");

        // //reset display
        // display = cv::Mat::zeros(800, 800, CV_8UC3);
        // for(const my_realtime_vdo_slam::VdoSlamScene& scene: map->scenes) {
        //     boost::shared_ptr<my_realtime_vdo_slam::VdoSlamScene> slam_scene_ptr = boost::make_shared<my_realtime_vdo_slam::VdoSlamScene>(scene);
    ...
    ros::Publisher RosVisualizer::create_viz_pub(ros::NodeHandle& nh) {
        return nh.advertise<my_realtime_vdo_slam::VdoSlamScene>("/vdoslam/output/scene", 10);
    }

    ros::Publisher RosVisualizer::create_map_update_pub(ros::NodeHandle& nh) {
        return nh.advertise<my_realtime_vdo_slam::VdoSlamMap>("/vdoslam/output/map", 10);
    }

    // bool RosVisualizer::update_spin(const my_realtime_vdo_slam::VdoSlamScenePtr& slam_scene) {
    //     publish_odom(slam_scene);
    ...
    void RosVisualizer::publish_odom(const my_realtime_vdo_slam::VdoSlamScenePtr& slam_scene) {
        nav_msgs::Odometry odom;
    ...
    // void RosVisualizer::publish_3D_viz(const my_realtime_vdo_slam::VdoSlamScenePtr& slam_scene) {
    //     visualization_msgs::MarkerArray marker_array;
    //     int i = 0;
    //     for (my_realtime_vdo_slam::VdoSceneObject& scene_object : slam_scene->objects) {
