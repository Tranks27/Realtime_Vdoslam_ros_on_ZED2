...
#include "vdo_slam_g2o/core/optimization_algorithm_factory.h"
...
namespace g2o {
  ...
  namespace
  {
    ...
    std::unique_ptr<BlockSolverBase> AllocateSolver()
    {
      ...
      return g2o::make_unique<BlockSolverPL<p, l>>(std::move(linearSolver).release());
    }
  }
  ...
  static OptimizationAlgorithm* createSolver(const std::string& fullSolverName)
  {
    ...
    if (methodName == "gn")
    {
      return new OptimizationAlgorithmGaussNewton(solverf->second().release());
    }
    else if (methodName == "lm")
    {
      return new OptimizationAlgorithmLevenberg(solverf->second().release());
    }
    else if (methodName == "dl")
    {
      return new OptimizationAlgorithmDogleg(solverf->second().release());
    }
